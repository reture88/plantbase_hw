...
describe('classifyRequest', () => {
  it('should detect a catalog question with no export intent', async () => {
    doGenerateMock.mockResolvedValueOnce(mockTextResponse('SZANDEK: KATALOGUS\nEXPORT: NEM', 12, 6))
  ...
  it('should classify a general plant-care question as knowledge_base', async () => {
    doGenerateMock.mockResolvedValueOnce(mockTextResponse('SZANDEK: TUDASBAZIS\nEXPORT: NEM', 10, 6))

    const result = await classifyRequest('Milyen gyakran öntözzem az aloe verát?', { apiKey: 'test-key', model: 'claude-test' })

    expect(result.intent).toBe('knowledge_base')
    expect(result.wantsFileExport).toBe(false)
  })

  it('should fail closed (catalog intent, no export) when the classifier call throws', async () => {
    ...
    expect(result).toEqual({ intent: 'catalog', wantsFileExport: false, usage: { inputTokens: 0, outputTokens: 0 } })
  })
})
