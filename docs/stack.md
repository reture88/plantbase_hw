...
- RAG-tudásbázis: pgvector (`vector(1536)`, HNSW-index) + `@ai-sdk/openai` (`text-embedding-3-small` embedding — az Anthropic nem kínál embedding-modellt). LLM-feladatok modellenként (ár/feladat-illeszkedés alapján, lásd `docs/architektura.md` 11. pont): Claude Haiku 4.5 (HyDE, grounded válaszadás), gpt-5.4-mini (rerank, szemantikus chunk-elemzés) — utóbbi kettő olcsóbb ítéleti/strukturált-kimenetes feladatra, ugyanazzal az `OPENAI_API_KEY`-vel, mint az embedding
- Egységes chat-orchestrátor: `web_search` (`@ai-sdk/anthropic` `webSearch_20250305`) kizárólag a tudásbázis-fallback lépésben — nem a katalógus-agent toolja
