...
### Hibajavítás: élő tesztelés közben az Excel-generálás "nem érkezett fájl a válaszban" hibával elszállt

Manuális tesztelés közben a `generateQuoteDocument` (`packages/core/src/agent/quote-document.ts`) a fenti hibával dobott kivételt, még olyan kérésnél is, ahol egyértelmű export-utasítás volt. Két, egymást erősítő ok azonosítva a `claude-api` skill dokumentációja alapján (nem találgatva):

- **`max_tokens` túl alacsony volt (4096).** A hivatalos Skills+code-execution minták (xlsx/pptx generálás) mind `max_tokens: 16000`-et használnak — az .xlsx elkészítése több kód-végrehajtási kört igényelhet (csomagtelepítés, szkript írása/futtatása), és egy alacsonyabb korlát elvághatja a választ a fájl elkészülte előtt (`stop_reason: "max_tokens"`), miközben a hibaüzenetünk ezt nem árulta el.
- **Nem volt kezelve a `pause_turn`.** A `code_execution` szerver-oldali tool a `web_search`-höz hasonlóan saját belső lépés-limitje miatt `pause_turn`-nel állhat meg; a kód egyetlen hívást indított, újraküldés nélkül, így egy ilyen megállás után sosem derült volna ki, hogy valójában folytatható lett volna.

**Javítás:** `QUOTE_MAX_TOKENS` 16000-re emelve; a függvény egy (max 5 körös, `QUOTE_MAX_TURNS`) ciklusra alakítva, ami `pause_turn` esetén az eddigi `messages`-t (a `response.content`-tel kiegészítve) újraküldi, ugyanúgy, ahogy az `ask-agent.ts` teszi a `web_search`-nél. A hibaüzenet mostantól tartalmazza a tényleges `response.stop_reason`-t, hogy hasonló eset legközelebb azonnal diagnosztizálható legyen naplózás/hibaüzenet alapján, kód-olvasás nélkül is. `quote-document.spec.ts` bővítve: `pause_turn`-folytatás tesztje, és a hibaüzenet `stop_reason`-tartalmának ellenőrzése.

### Átállás `@anthropic-ai/sdk`-ról Vercel AI SDK-ra

A user kifejezett kérésére a `packages/core` agent-rétege (`ask-agent.ts`, `request-classifier.ts`, `quote-document.ts`, a tool-fájlok) lecserélődött a hivatalos `@anthropic-ai/sdk`-ról a Vercel AI SDK-ra (`ai@^7.0.34` + `@ai-sdk/anthropic@^4.0.18`) — a cél szélesebb, jövőben bővíthető eszközkészlet, a működés és minden funkció (SQL-agent, `listCategories`, `web_search`-kapuzás, Excel-export-kapuzás, JSONL-naplózás, `--show-prompt`) változatlansága mellett.

**Kutatás (Context7 `/vercel/ai` + a ténylegesen telepített csomag `.d.ts`-ei, nem találgatva):** Context7 egy régebbi (v6-os) doksit indexelt, a valós `npm view`-val ellenőrzött legfrissebb stabil verzió `ai@7.x` — a pontos tool-factory nevek (`codeExecution_20260120`, `webSearch_20260209`) és a `generateText` eredmény-alakja (`toolResults[i].output`, `responseMessages`) ezért a ténylegesen telepített `node_modules`-beli típusdefiníciókból lettek megerősítve, nem a doksi-példákból másolva.

- **A kézzel írt tool-use loop (`for` ciklus + manuális `tool_use`/`tool_result` blokk-kezelés) helyett `generateText({..., tools, stopWhen: isStepCount(8)})`** — az SDK maga hajtja végre a többlépéses tool-hívást.
- **`runSql`/`listCategories`**: a korábbi `{toolDefinition, createHandler}` pár egyetlen `tool({inputSchema: zodSéma, execute})` hívássá egyszerűsödött. A naplózáshoz szükséges pontos időzítés/hibaüzenet mostantól magában a tool `execute`-jában készül el, egy megosztott `logSink: ToolCallLogEntry[]` tömbbe írva — az AI SDK a saját tool-loopja miatt ezt már nem az `ask-agent.ts` szintjén, központilag figyeli.
- **`web_search`**: saját `webSearchToolDefinition` konstans helyett `anthropic.tools.webSearch_20260209()`.
- **`quote-document.ts`**: `anthropic.tools.codeExecution_20260120()` + `providerOptions.anthropic.container.skills: [{type:'anthropic', skillId:'xlsx', version:'latest'}]` (camelCase `skillId`, nem `skill_id`). **Élő teszteléskor talált és javított hiba**: a `code_execution` tool több, egymástól eltérő `output`-alakot ad vissza lépésenként (pl. egy SKILL.md megnézésekor `content` egy STRING, nem tömb) — a fájl-keresés kódja ezt eredetileg feltétel nélkül `.find()`-olta, ami `TypeError`-t dobott. Javítás: `Array.isArray(output?.content)` ellenőrzés, mielőtt a `.find()` lefut; a regressziós teszt is bővült egy ilyen vegyes-alakú tool-result-tal.
- **A generált fájl letöltése NEM az AI SDK-n keresztül történik**: az `anthropic.files()` (`FilesV4`) csak `uploadFile`-t exportál, letöltő metódust nem (megerősítve a telepített `@ai-sdk/provider` típusaiból). Ezért `downloadQuoteDocument` és a fájl-metaadat lekérése közvetlen `fetch`-csel hívja Anthropic hivatalos Files API REST végpontjait (`GET /v1/files/{id}` és `GET /v1/files/{id}/content`), a `@ai-sdk/anthropic` csomag saját, ugyanerre használt fejlécértékeivel (`anthropic-version: 2023-06-01`, `anthropic-beta: files-api-2025-04-14`, szintén a telepített csomag forrásából megerősítve).
- **Tesztelés**: minden `packages/core/src/agent/*.spec.ts` átírva `ai/test`-ből `MockLanguageModelV4`-re — a korábbi `vi.mock('@anthropic-ai/sdk', ...)` minta helyett a `@ai-sdk/anthropic` modult mockoljuk (`vi.mock` + `importOriginal`, csak a `createAnthropic`-ot felülírva, hogy a valós `anthropic.tools.*` factory-k — amik önmagukban nem hálózati hívások — érintetlenek maradjanak). A `doGenerate` mock ugyanúgy `mockResolvedValueOnce`-ekkel láncolható, mint korábban.
- **Ismert, de nem hiba**: élő teszteléskor egy plant-care kérdésnél a modell (Claude Sonnet 4.6) a `web_search`-öt Anthropic "programmatic tool calling" mechanizmusán (`code_execution` tool egy generált szkripttel) keresztül hívta meg, annak ellenére, hogy `code_execution` nincs a `askAgent` tools listájában ajánlva SQL-agent módban — ez modell/API-szintű, a mi tool-listánktól független viselkedés, a végeredmény (helyes válasz) nem sérült.
- **Élő smoke teszt** (`pnpm cli ask ...`, 3 eset: sima katalógus-kérdés, web_search-öt igénylő növénygondozási kérdés, export-utasítást tartalmazó kérdés) mindhárom esetben helyesen működött a javítás után — utóbbi ténylegesen létrehozott egy valós, megnyitható `.xlsx` fájlt a `quotes/`-ban.
- Dokumentáció frissítve: `docs/architektura.md` (3. és 8. pont), `docs/stack.md` (Agent sor).

### RAG-tudásbázis (embedding, HyDE, rerank, grounding) + egységes chat + PDF-export

A user három nagyobb, egymásra épülő kérést adott ezen a napon:

1. **Első kör**: teljes RAG-pipeline a `seed/knowledge/` (202 db, "Ask the Sill" növényápolási cikk) tudásbázisra — kétlépcsős chunkolás (bekezdés-split + LLM-alapú szemantikus finomítás), pgvector-alapú vektor-keresés, HyDE, rerank, grounded válaszadás explicit elutasítási logikával, `ingest-knowledge` CLI-parancs, `apps/api` (Fastify) + `apps/web` (React) két külön chat-fülként (katalógus / tudásbázis).
2. **Streaming-átállás**: mindkét pipeline `generateText`/`generateObject`-ről `streamText`-re váltva, valódi SSE token-streaminggel a böngészőig — a RAG-elutasítás jelzőjét (`NINCS_ELEG_INFORMACIO`) egy puffereléssel oldottuk meg, hogy sose villanjon fel nyersen a kliensnél.
3. **Egységes chat + web_search-fallback + PDF + modellváltás**: a két fület egyetlen chat-ablakká vontuk össze — egy klasszifikáció (`intent: catalog | knowledge_base`) dönt, a `web_search` a katalógus-agensről áthelyeződött a tudásbázis-ág fallback-lépésére (`web-fallback-agent.ts`, csak akkor fut, ha a RAG bizonyítottan nem tud válaszolni, előtte egy feltűnő `notice` esemény jelzi ezt a felhasználónak). PDF-export bevezetve (Anthropic hivatalos `pdf` Agent Skill, ugyanaz a mechanizmus mint az xlsx-nél) a tudásbázis/web_search-válaszokhoz, Excel marad a katalógus-válaszoknál. A rerank és a szemantikus chunk-split modellje Claude Haiku 4.5-ről gpt-5.4-mini-re váltott (hivatalos árak alapján: $0.75/$4.50 vs $1/$5 per MTok, a HyDE és a végső grounded válaszadás Claude Haiku-n maradt) — az `ingest-knowledge` parancsnak emiatt már nincs szüksége Anthropic-kulcsra.

**Élesben talált és javított hibák eközben:**
- `web_search` (`webSearch_20260209`) nem működött `claude-haiku-4-5`-tel — az Anthropic API "programmatic tool calling"-ot igényelt volna hozzá, amit a Haiku modellcsalád nem támogat, és az AI SDK erre a tool-verzióra nem enged `allowed_callers` felülbírálást. Megoldás: visszaváltás a régebbi, ehhez a kerethez nem kötött `webSearch_20250305`-re.
