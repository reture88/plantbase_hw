...
export function registerAskCommand(program: Command, agentConfig: AgentConfig, logger: JsonlLogger): void {
  program
    .command('ask')
    ...
    .action(async (question: string, options: { showPrompt?: boolean }) => {
      const result = await askAgent(question, { ...agentConfig, logger })
